...
# doesn't recursively dup any arrays contained therein. So the dup
# method creates one new array, but just copies over references to the
# original interior arrays.
#
...
# duplication of arrays. Don't worry about deep-duping (or
# regular-duping) other types of mutable objects (like strings, hashes,
# instances of custom classes, etc.), since this would require that we
# implement a deep dup method for each of those classes, as well.
#
# It's okay to iterate over array elements using the normal each for this
# one :-)
#
# You should be able to handle "mixed" arrays.
